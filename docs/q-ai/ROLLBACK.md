...
Q-AI is disabled by default. To keep existing behavior unchanged, leave
`Q_AI_ENABLED` unset or set it to `false`. `Q_AI_SHADOW=true` is reserved for a
future comparison path and does not replace the existing Studio response.

Rollback is immediate: remove/disable the Q-AI environment flag, restart the
