...
export class QAIOrchestratorService {
  ...
  async evaluate(request: QAIRequest): Promise<Decision> {
    ...
    const perRound = Math.min(request.maxModels ?? config.maxParallelModels, config.maxParallelModels);
    const selected = eligible.slice(0, perRound * Math.min(request.maxRounds ?? config.maxRounds, config.maxRounds));
    const prior = initialProbabilityField(selected);
    const allEvidence: ModelEvidence[] = [];
    const started = Date.now(); const globalDeadline = started + config.timeoutMs * config.maxRounds; let rounds = 0; let measured: Decision | undefined;
    while (rounds < Math.min(request.maxRounds ?? config.maxRounds, config.maxRounds)) {
      rounds++;
      ...
      if (measured.earlyTermination || rounds >= config.maxRounds || measured.totalCost >= config.maxCost || Date.now() >= globalDeadline) break;
    }
