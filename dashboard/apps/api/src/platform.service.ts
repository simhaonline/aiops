...
@Injectable() export class PlatformService {
  ...
  async telemetry(){try{const r=await fetch(`${process.env.AIOPS_TELEMETRY_URL??"http://127.0.0.1:11082"}/snapshot`,{signal:AbortSignal.timeout(1500)});if(r.ok)return await r.json();}catch{}return {host:"unavailable",uptime:"—",load:0,memory:0,managers:[]};}
  async projects(){
